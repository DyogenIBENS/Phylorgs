...
### This bit sets 'ini.time' and should result in no negative branch lengths
    ...
    repeat {
        ...
        ## Better convert the ages to ranks (integers):
        #uniq.calibs.after.NA <- sort(unique(unlist(calibs.after.NA)))
        #ranks.after.NA <- lapply(calibs.after.NA, findInterval, uniq.calibs.after.NA)
        ## All numbers converted to strings must be left-padded with zeros if necessary.
        #fmt <- paste0('%0', nchar(length(uniq.calibs.after.NA)), 'd')
        #sort.key <- lapply(ranks.after.NA, function(r) paste0(sprintf(fmt, r),
        #                                                      collapse=''))
        #o <- order(unlist(sort.key))
